...
def imageFolder2mpeg(frames, output_path='./output_video.mpeg', fps=30.0):
    ...
    frame_Height, frame_Width = cv2.imread(frames[0]).shape[:2]
...
def getVideo(vid1, vid2, vid3):
    ...
    for i in range(len(vid1frames)):
        ...
        total_x = int(480 + 2*abs(T0[0, 2]))
        total_y = int(360 + 50 + 2*abs(T0[1, 2]))
        ...
        final = img_warped0 + (img_warped0 == 0)*img_warped1 + (img_warped1 == 0)*img_warped2
        finalframes.append(final)
    output_p = './output_video.mpeg'
    imageFolder2mpeg(finalframes, output_path='./output_video.mpeg', fps=30.0)
    return output_p